...
function afficher_coffresProches(donnees) {
 	for (var i = 0; i < 2; i++) {
		...
		$.get(url + "joueurs/"+coffre_idPirate, null, afficher_coffre_joueurs, "json");	
	}
}
...
function afficher_coffre_joueurs(donnees){
	...
	var pirate_idClan=donnees['IDCLAN'];
 	var res='<tr><td class="table_sousTitre">'+pirate_pseudo+'</td><td><button class="btn_ramasser" id="ramasser">Ramasser</button></td></tr>';
